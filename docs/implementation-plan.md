...
|---|------|----------------------|-------|---------|
| **M1.1** | **Backend skeleton** | Minimal API solution; `Features/*` + `Shared/*` folders scaffolded (empty stubs ok); MSSQL connection wired via `Shared/Database`; `GET /health` returns ok; `IHttpClientFactory` registered; config for JWT key + executor limits (timeout, max size) read from settings. Runs with `dotnet run`. | Smoke test: app boots, `/health` 200. | QA |
